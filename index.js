...
export default{
  async fetch(request,env){
    ...
    if(path==='/api/health'){
      await ensureSchema(env);
      return json({ok:true,version:'0.7.0',d1:!!env.DB,sharepointConfigured:!!env.SHAREPOINT_FILE_URL,authConfigured:env.DB?await authConfigured(env):false});
    }
    ...
    if(path==='/api/admin/settings'){
      ...
      return json({ok:true,version:'0.7.0',sharepointConfigured:!!env.SHAREPOINT_FILE_URL,d1:!!env.DB,source:overview.source,authUser:auth.user});
    }
